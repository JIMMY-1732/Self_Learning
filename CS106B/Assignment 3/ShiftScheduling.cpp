...
Set<Shift> HVSF_Helper(const Set<Shift>& shifts, Set<Shift> combinator, int maxHours){
    Set <Shift> rest_of_shifts;
    Set <Shift> test1;
    Set <Shift> test2;
    int flag = true;
    int sum = 0;
    ...
    if(shifts.size() == 0){
        ...
        rest_of_shifts = shifts - first;

        for(auto shift : combinator){
            if(overlapsWith(first, shift)){
                flag = false;
                break;
            }
        }

        for(auto shift : combinator + first){
            sum += lengthOf(shift);
            if(sum > maxHours){
                flag = false;
                break;
        ...
        if(flag){
            test1 = HVSF_Helper(rest_of_shifts, combinator + first, maxHours);
            test2 = HVSF_Helper(rest_of_shifts, combinator, maxHours);
            for(auto shift : test1){
                valuesum += valueOf(shift);
            }
            for(auto shift : test2){
                valuesum -= valueOf(shift);
            }
            if(valuesum >= 0)
                return test1;
            ...
            return HVSF_Helper(rest_of_shifts, combinator, maxHours);
    }
}
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    if(maxHours == 0){
        ...
        best_schedule = HVSF_Helper(shifts, {}, maxHours);
