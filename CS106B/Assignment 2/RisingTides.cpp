...
bool alreadyexist(Queue <GridLocation> Queue1,GridLocation Location){
    while(!Queue1.isEmpty()){
        GridLocation locate = Queue1.dequeue();
        if(locate == Location){
            return true;
        }
    }
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Queue <GridLocation> floodable;
    Queue <GridLocation> flooded;
    Grid <bool> result;
    GridLocation location;
    GridLocation temp;

    result = Grid(terrain.numRows(),terrain.numCols(),false);

    for(GridLocation locate : sources){
        if(terrain[locate] <= height){
            flooded.enqueue(locate);
            floodable.enqueue(locate);
            result[locate] = true;
        }
    }


    while(!floodable.isEmpty()){

        location = floodable.dequeue();

        if(location.row - 1 >= 0){
            temp = {location.row - 1,location.col};
            if( terrain[temp] <= height && !((alreadyexist(flooded,temp) || alreadyexist(floodable,temp)))){
                floodable.enqueue(temp);
                flooded.enqueue(temp);
                result[temp] = true;
            }
        }

        if(location.row + 1 < terrain.numRows()){
            temp = {location.row + 1,location.col};
            if(terrain[temp] <= height && !((alreadyexist(flooded,temp) || alreadyexist(floodable,temp)))){
                floodable.enqueue(temp);
                flooded.enqueue(temp);
        ...
        if(location.col - 1 >= 0){
            temp = {location.row,location.col - 1};
            if(terrain[temp] <= height && !((alreadyexist(flooded,temp) || alreadyexist(floodable,temp)))){
                floodable.enqueue(temp);
                flooded.enqueue(temp);
                result[temp] = true;
        ...
        if(location.col + 1 < terrain.numCols()){
            temp = {location.row,location.col + 1};
            if(terrain[temp] <= height && !((alreadyexist(flooded,temp) || alreadyexist(floodable,temp)))){
                floodable.enqueue(temp);
                flooded.enqueue(temp);
